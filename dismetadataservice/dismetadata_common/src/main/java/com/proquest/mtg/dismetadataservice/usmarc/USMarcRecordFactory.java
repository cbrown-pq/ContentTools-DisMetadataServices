...
public class USMarcRecordFactory extends MarcRecordFactoryBase {
	...
	private void handleCatalogingSource() {
		addField(MarcTags.kCatalogingSource,
				makeFieldDataFrom(' ', ' ', 'a', kSystemPQPrefix) + makeFieldDataFrom('c', kSystemPQPrefix));
	}
		...
		if (null != authors && !authors.isEmpty()) {
			...
			if (null != authorFullname) {
				if (null != orcID && !orcID.isEmpty()) {
					...
					addField(MarcTags.kAuthor, makeFieldDataFrom('1', ' ', 'a', endWithPeriod(authorFullname))
	...
	private void handleDissertationNote() {
		...
		if (null != disNote) {
			addField(MarcTags.kDissertationNote, makeFieldDataFrom(' ', ' ', 'a', "Thesis " + endWithPeriod(disNote)));
		}
	}
	...
	private void handleGeneralNoteForAdvisor() {
		...
		if (null != advisors) {
			...
			addField(MarcTags.kGeneralNote, makeFieldDataFrom(' ', ' ', 'a', advisor));
		}
	}

	private void handlePageCount() {
		if (null != curMetaData.getPageCount() && !curMetaData.getPageCount().isEmpty()) {
			addField(MarcTags.kPageCount, makeFieldDataFrom(' ', ' ', 'a', curMetaData.getPageCount() + " p."));
		}
	}
	...
	private void handleMultipleAuthors() {
		...
		if (null != authors && !authors.isEmpty()) {
			for (Author curAuthor : authors) {
				...
				if (authorSequence > 1) {
					addField(MarcTags.kMulitpleAuthor, makeFieldDataFrom('1', '0', 'a', curAuthor.getAuthorFullName())
	...
	private void handleCorporateEntry() {
		...
		if (null != schoolName && !schoolName.isEmpty()) {
			if (null != deptName && !deptName.isEmpty())
				...
				addField(MarcTags.kCorporatename, makeFieldDataFrom('2', '0', 'a', schoolName + "."));
		}
	...
	private void handleDisserationLanguage() {
		...
		if (dissLanguages != null && !dissLanguages.isEmpty()) {
			...
			addField(MarcTags.kDissertationLanguage, makeFieldDataFrom(' ', ' ', 'a',
	...
	private void handlePublisher() {
		...
		if (null != authors && !authors.isEmpty()) {
			...
			if (null != degreeYear && !degreeYear.isEmpty()) {
				...
				addField(MarcTags.kPublisher, makeFieldDataFrom(' ', '1', 'a',
