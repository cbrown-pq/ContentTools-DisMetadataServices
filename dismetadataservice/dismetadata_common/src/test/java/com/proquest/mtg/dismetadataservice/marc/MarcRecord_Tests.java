...
public class MarcRecord_Tests {
	...
	public void setUp() throws Exception {
		...
		fieldTitle = new MarcField(MarcTags.kTitle,
				"A systematic study of the Leucanthemum-Chrysanthemum complex in North Africa.");
		fieldAbstract1 = new MarcField(MarcTags.kAbstract,
				"Abstract not available.");
		fieldAbstract2 = new MarcField(MarcTags.kAbstract,
				"Abstract still not available.");
		fieldAbstract3 = new MarcField(MarcTags.kAbstract,
				"Can't find the abstract.");
		fieldUrl = new MarcField(MarcTags.kUrl,
				"http://mystuff.com/host?id:441720");
		fieldUrl = new MarcField(MarcTags.kUrl,
				"http://mystuff.com/host?id:441720");

		fields = Lists.newArrayList(fieldId, fieldAuthor, fieldTitle,
				fieldAbstract1, fieldAbstract2, fieldAbstract3, fieldUrl);

		target = new MarcRecord(status, type, level, characterEncoding, ' ',
				' ', fields);
	}
	...
	public void getFieldsWithTag_SingleMatch() throws Exception {
		...
		assertThat(target.getFieldsMatchingTag(MarcTags.kAuthor),
				is(matchingFields));
	}

	@Test
	public void getFieldsWithTag_MultiMatch() throws Exception {
		List<MarcField> matchingFields = Lists.newArrayList(fieldAbstract1,
				fieldAbstract2, fieldAbstract3);

		assertThat(target.getFieldsMatchingTag(MarcTags.kAbstract),
				is(matchingFields));
	}
	...
	public void getFieldsWithTag_NoMatch() throws Exception {
		...
		assertThat(
				target.getFieldsMatchingTag(MarcTags.kEnglishTranslationOfTitle),
				is(matchingFields));
	}
	...
	public void getFirstFieldWithTag_NoMatch() throws Exception {
		assertThat(
				target.getFirstFieldMatchingTag(MarcTags.kEnglishTranslationOfTitle),
				nullValue());
	}

	@Test
	public void getFirstFieldWithTag() throws Exception {
		assertThat(target.getFirstFieldMatchingTag(MarcTags.kAbstract),
				is(fieldAbstract1));
	}
	...
	public void hasCorrect_tags() throws Exception {
		List<String> expectedTags = Lists.newArrayList(MarcTags.kRecId,
				MarcTags.kAuthor, MarcTags.kTitle, MarcTags.kAbstract,
				MarcTags.kAbstract, MarcTags.kAbstract, MarcTags.kUrl);
	...
	public void sortsTags_WhenFieldsAddedOutOfOrder() throws Exception {
		List<MarcField> fieldsOutOfOrder = Lists.newArrayList(fieldAbstract1,
				fieldAuthor, fieldAbstract2, fieldTitle, fieldAbstract3,
				fieldUrl, fieldId);
